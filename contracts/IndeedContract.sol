pragma solidity ^0.5.0;
    ...
    ///@param occupantDate is the date the person took occupancy
    struct UserInfo {
            string name;
            address userAddress;
            string IDhash;
    ...
    ///@return three strings that identify the senders user details as an array - this is the same details in the user struct
    function getUser () public view returns (string memory, string memory, string memory) {
        uint256 propNum = addressToUser[msg.sender];
        return(users[propNum].name, users[propNum].IDhash, users[propNum].occupantDate);
    }
}